...
import { 
  Cpu, Terminal, Target, ArrowRight, ShieldCheck, 
  Music, Monitor, Gift, Rocket, BadgePercent, Wrench, HardDrive, Network
} from 'lucide-react';
...
const panelsData = [
  {
    ...
    cards: [
      ...
      { id: 'EDU-02', title: 'DEVELOPPEMENT', tag: 'EXPERT', icon: Terminal, desc: 'Conception d’applications modernes.', color: '#00f2fe', image: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=800&auto=format&fit=crop' },
      { id: 'EDU-03', title: 'DEPLOIEMENT', tag: 'CRÉATION', icon: Music, desc: 'Installation et mise en place des équipements.', color: '#ff0055', image: 'https://images.unsplash.com/photo-1514525253361-bee8a187499b?q=80&w=800&auto=format&fit=crop' },
    ]
  },
  {
    id: "services",
  ...
  },
  {
    id: "vdi",
    unit: "UNIT_03",
    category: "INSTALLATION_VDI_&_INFRASTRUCTURES",
    layout: "triple",
    cards: [
      { id: 'VDI-01', title: 'CÂBLAGE STRUCTURÉ', tag: 'CAT6A / CAT7', icon: Network, desc: 'Pose de goulottes et tirage de câbles Ethernet blindés.', color: '#a855f7', image: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?q=80&w=800&auto=format&fit=crop' },
      { id: 'VDI-02', title: 'INSTALLATION VDI ET BAIE DE BRASSAGE', tag: 'CLEAN-UP', icon: HardDrive, desc: 'Installation VDI (camera, CCTV, equipement informatique), Rangement, étiquetage et raccordement des switchs.', color: '#00f2fe', image: 'https://images.unsplash.com/photo-1563770660941-20978e870e26?q=80&w=800&auto=format&fit=crop' },
      { id: 'VDI-03', title: 'CERTIFICATION', tag: 'RECETTE', icon: Wrench, desc: 'Tests de performance et validation des liaisons.', color: '#ff0055', image: 'https://images.unsplash.com/photo-1600132806370-bf17e65e942f?q=80&w=800&auto=format&fit=crop' },
    ]
  },
  {
    id: "projet",
...
const Shop = () => {
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      // Mise à jour des noms des sections pour correspondre à ton composant de navigation NavShop
      const sectionNames = ['ACCUEIL', 'SERVICES', 'INFRA VDI', 'PROMOTIONS'];
      ...
      <div ref={slider} className="flex flex-col md:flex-row flex-nowrap min-h-screen md:w-max">
        {panelsData.map((panel, pIndex) => (
          <section key={pIndex} id={panel.id} className="panel w-full md:w-screen min-h-screen flex-shrink-0 flex flex-col pt-32 pb-10 px-4 md:px-8 relative border-b md:border-b-0 md:border-r border-white/5">
            ...
            {/* Arrière-plan stylisé avec l'identifiant de l'unité */}
            <div className="absolute top-24 right-10 text-[10rem] font-black text-white/[0.02] pointer-events-none select-none italic uppercase leading-none z-0">
              {panel.unit}
            </div>
              ...
              {panel.cards.map((card) => {
                ...
                return (
                  <div key={card.id} className="anim-card group relative border border-white/10 bg-[#111] hover:border-[#00f2fe]/40 transition-all duration-500 overflow-hidden flex flex-col cursor-pointer min-h-[250px]">
                    ...
                    {/* Image d'arrière-plan avec opacité gérée au survol */}
                    <div className="absolute inset-0 z-0">
                      <img src={card.image} alt="" className="w-full h-full object-cover opacity-20 group-hover:opacity-40 transition-opacity duration-500" />
                    ...
                    <div className="p-6 relative z-10 flex flex-col h-full justify-between">
                      <div className="flex justify-between items-start">
                        <div className="p-2 bg-black/70 border border-white/20">
                          <Icon size={20} style={{ color: card.color }} />
                        </div>
                        <span className="text-[8px] font-bold text-white bg-black/80 px-2 py-0.5 border border-white/10 uppercase tracking-widest">{card.tag}</span>
                      </div>

                      <div className="bg-black/70 backdrop-blur-sm p-4 border-l-2 mt-4" style={{ borderColor: card.color }}>
                        <h3 className="text-xl font-black uppercase tracking-wide">{card.title}</h3>
                        <p className="text-white/70 text-[11px] mt-1 font-sans leading-relaxed">{card.desc}</p>
                      </div>

                      <div className="flex justify-between items-center mt-4">
                        <span className="text-[8px] text-white/40 tracking-wider">{card.id}</span>
                        <ArrowRight size={14} className="text-[#00f2fe] transform group-hover:translate-x-1 transition-transform" />
                      </div>
                    </div>
